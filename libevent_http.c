...
{
    if(strcasecmp("GET", method) == 0){     //字符串比较函数，相等返回0
        //get method ...
        strdecode(path, path);
        char *pf = &path[1];    //指向要打开的文件
        //判断字符串是否相等
        //解析得到的path开头是带/的，比如test.c，其实是/test.c
        if(strcmp(path, "/") == 0 || strcmp(path, "/.") == 0)   //如果没有请求具体文件
            //将pf转化成./，表示当前文件夹。
        {
            pf="./";
        }
...
{
    char* dot;
    //返回.开始后面的所有字符串，即返回文件格式后缀
    dot = strrchr(name, '.');	//自右向左查找‘.’字符;如不存在返回NULL

    if (dot == (char*)0) //没找到，返回无格式正文
